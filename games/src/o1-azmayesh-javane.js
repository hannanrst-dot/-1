...
/* ═══════════════════════════════════════════════════════════════════════
   ...
   ▸ نور برای جوانه‌زدنِ کاهو لازم نیست؛ اثرش کوچک است. عمداً گذاشته
     شده تا بچّه ببیند هر عاملی که عوض می‌کنی لزوماً مهم نیست.
...
/** نور برای کاهو تقریباً بی‌اثر است — عمداً یک عاملِ کم‌اثر داریم. */
const fLight = (on) => (on ? 1 : .94);
...
function makePot() {
  return { seeds: 20, temp: 25, water: .55, light: true, day: 0 };
}
const potRate = (p) => .62 * fTemp(p.temp) * fWater(p.water) * fLight(p.light);
...
function conclude(i) {
  ...
  const a = potCount(S.pot[0]), b = potCount(S.pot[1]);
  const win = a === b ? -1 : (a > b ? 0 : 1);
  if (win === -1) {
    S.verdict = { pick: i, ok: true, tie: true };
    sfx.good();
    tip('هر دو یکی شد — پس این عامل اثری نداشت.');
    return;
  }
  S.verdict = { pick: i, ok: i === win, win };
  if (i === win) { sfx.win(); tip('درست است؛ همین ظرف بیشتر جوانه داد.'); }
  else { sfx.nope(); tip('به جدول نگاه کن: کدام ستون عددِ بزرگ‌تری دارد؟'); }
  S.stepI = 4;
}
...
const CTRL = ['seeds', 'temp', 'water', 'light'];
const CTRL_N = ['دانه', 'دما', 'آبِ خاک', 'نور'];
function ctrlRow(i, k) {
  const b = potBox(i);
  return { x: b.x + 12, y: b.y + 366 + k * 57, w: b.w - 24, h: 50 };
}
function trackOf(i, k) {
  const r = ctrlRow(i, k);
  return { x: r.x + 78, y: r.y + 25, w: r.w - 132 };
}
function qChip(i) {
  return { x: SIDE.x + 22 + i * 166, y: SIDE.y + 62, w: 154, h: 58 };
}
...
cv.addEventListener('pointermove', (e) => {
  ...
  for (let i = 0; i < 3; i++) if (inRect(p, qChip(i))) S.hover = { k: 'q', i };
...
cv.addEventListener('pointerdown', (e) => {
  const p = toStage(e);
  for (let i = 0; i < 3; i++) if (inRect(p, qChip(i))) {
...
function drawPot(i) {
  ...
  const d = diffs();
  if (d.length) {
    const names = { seeds: 'دانه', temp: 'دما', water: 'آب', light: 'نور' };
  ...
  ctx.strokeStyle = 'rgba(160, 200, 210, .8)'; ctx.lineWidth = 3;
  ...
  for (let k = 0; k < 4; k++) {
    ...
    text(CTRL_N[k], r.x + r.w - 8, r.y + 16, { size: 13, color: P.inkSoft, align: 'right' });
    if (key === 'light') {
      const on = p.light;
      const bx = t2.x + t2.w - 62;
      ctx.fillStyle = on ? P.gold : 'rgba(37, 48, 42, .2)';
      ctx.beginPath(); rrPath(bx, r.y + 12, 58, 26, 13); ctx.fill();
      ctx.fillStyle = '#fff';
      ctx.beginPath(); ctx.arc(on ? bx + 44 : bx + 14, r.y + 25, 10, 0, TAU); ctx.fill();
      text(on ? 'روشن' : 'تاریک', t2.x + 26, r.y + 25, { size: 14, color: P.ink });
    ...
    numText(val, r.x + 34, r.y + 25, { size: key === 'water' ? 15 : 19, color: P.ink,
...
function drawSide() {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    text(QUESTIONS[i].n, b.x + b.w / 2, b.y + b.h / 2, { size: 18, family: 'Lalezar', color: on ? P.accent : P.ink });
  }
  /* نشانِ منصفانه بودن */
  const d = diffs();
  const ok = d.length === 1;
