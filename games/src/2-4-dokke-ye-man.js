...
function spawnCustomer() {
  ...
  if (!stocked.length || S.queue.length >= 3) return;
  // چند ترکیب را امتحان می‌کنیم تا یکی پیدا شود که باقی‌اش ساختنی باشد.
  // اگر هیچ‌کدام نشد، این لحظه مشتری نمی‌آید و بچه فرصت نفس‌کشیدن دارد.
  let pickItem = null, paidV = 0;
  for (let tries = 0; tries < 8; tries++) {
    ...
    if (canMakeChange(v - cand.good.price)) { pickItem = cand; paidV = v; break; }
  }
  if (!pickItem) return;
  const price = pickItem.good.price;
  const paid = paidV === 20000 ? [10000, 10000] : [paidV];
  const patience = (14 - S.day * 1.2) * (S.upgrades.includes('saye') ? 1.45 : 1);
  S.queue.push({
    id: Math.random(),
    good: pickItem.good,
    price,
    paid,
    change: paidV - price,
    patience: 1, maxP: Math.max(6, patience),
...
function step(dt) {
  ...
  if (S.phase === 'day') {
    S.spawnT -= dt;
    const rate = 3.4 - S.day * .35 - (S.upgrades.includes('tablo') ? .5 : 0);
    if (S.spawnT <= 0 && S.served + S.queue.length < S.target) {
      spawnCustomer();
      S.spawnT = Math.max(1.2, rate);
    }
...
function drawOrderBubble(c) {
  ...
  ctx.fill();
  text(left > 0 ? `باقی: ${fa(c.change)} تومان   (${fa(left)} مانده)`
     : left === 0 ? `باقی کامل شد: ${fa(c.change)} تومان`
