...
function pullLever() {
  ...
  S.spin = { seq, i: 0, t: 0, dur: .42, from: S.rot, to: S.rot };
  nextSpin(S.spin);
  sfx.slide();
}
...
function nextSpin(sp) {
  const i = sp.seq[sp.i];
  const step2 = TAU / S.n;
  /* عقربه بالای چرخ است؛ چرخ را طوری می‌چرخانیم که قسمتِ i زیرش بیفتد */
...
cv.addEventListener('pointermove', (e) => {
  ...
  if (S.drawing) { paintSector(sectorAt(p)); return; }
  S.hover = null;
  const s = sectorAt(p);
...
function drawWheel() {
  ...
  const stepA = TAU / S.n;
  for (let i = 0; i < S.n; i++) {
    const a0 = S.rot + i * stepA, a1 = a0 + stepA;
    const k = S.sec[i];
    ctx.beginPath();
    ctx.moveTo(x, y);
    ctx.arc(x, y, r, a0, a1);
  ...
  /* عقربه بالای چرخ */
  const bob = S.spin ? Math.sin(S.t * 40) * 2 : 0;
  ctx.fillStyle = '#22182e';
  ctx.beginPath();
  ctx.moveTo(x - 20, y - r - 34 + bob); ctx.lineTo(x + 20, y - r - 34 + bob);
  ctx.lineTo(x, y - r + 12 + bob);
  ctx.closePath(); ctx.fill();
  ctx.fillStyle = P.brassLt;
  ctx.beginPath();
  ctx.moveTo(x - 15, y - r - 32 + bob); ctx.lineTo(x + 15, y - r - 32 + bob);
