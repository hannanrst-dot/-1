...
`peyvaste` (نیم‌فاصله) · `twins` (دوقلوهای هم‌آوا)

### حالت‌های بازی در مأموریت

`word_hunt` · `letter_snipe` · `sentence_hunt` · `twin_words` · `word_forge` ·
`word_rescue` · `monster_combat`

`twin_words` فقط با دستهٔ `twins` کار می‌کند (جفت‌های هم‌آوا). اگر آن دسته را
نخواسته باشید، بازی این حالت را خودش از برنامهٔ آزمون کنار می‌گذارد.
...
سه حالت در مأموریت نمی‌آیند و فقط در بازی آزاد هستند: «نبرد با غول» (صحنهٔ
پایانی مرحله‌محور)، «حملهٔ زمان‌دار» (تایمر مستقل که با ساعت آزمون تداخل
می‌کند) و «دفاع از دروازه» (هر دور یک موج چندواژه‌ای دارد و با شمارشِ
«هر دور، یک پرسش» جور درنمی‌آید).
