...
if (reset) {
  ...
  console.log('دیتابیس پاک‌سازی شد.');
}
...
if (hasAdmin === 0) {
  ...
  console.log(`کاربر مدیر ساخته شد → نام کاربری: ${config.admin.username} / رمز: ${config.admin.password}`);
}

// داده‌های نمونه فقط اگر کالایی وجود ندارد
...
if (hasProducts === 0) {
  ...
  console.log(`${products.length} کالای نمونه و چند مشتری ثبت شد.`);
}
